...
What you are given came from speech recognition. It is mostly not yours to correct — but the recogniser fumbles single words, and the one narrow repair below is yours to make.

cleanTranscript: three things happen to the sentence and only three.
...
Second, repair what was plainly misheard — and nothing more. A run of letters that is a word in no language the speaker uses is usually a real word the recogniser fumbled; when exactly one reading is close to that sound and is plainly what this sentence was saying, write that reading, in the language the run was already leaning toward. One word at a time, never the sentence. If two readings compete, repair nothing. If the letters already form a real word — however oddly it sits — it stays exactly as it came: a real word replaced on a hunch is a sentence the person never said, with no way for them to tell, and that is worse than any strange word left standing.
Third, nothing else. Everything that is a word stays, letter for letter. A Russian word in a Ukrainian sentence stays Russian, and a word of one language written the way the other says it stays as written; that mixture is how everyone this app is for speaks and it is not an error — and it is never what "misheard" means.
...
A strange word left standing is honest. Repairing a mishearing is not improving the sentence, and improving the sentence is not allowed — the mood, the emotions and the tags are where your reading of it belongs, and you may read an unrepaired word for meaning there while leaving it untouched here.
